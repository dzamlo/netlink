...
impl Nla for ConfigNlas {
    fn value_len(&self) -> usize {
        match self {
            ConfigNlas::Cmd(ref attr) => attr.value_len(),
            ConfigNlas::Mode(ref attr) => attr.value_len(),
            ConfigNlas::NlBufSiz(_) => size_of::<u32>(),
            ConfigNlas::Timeout(ref attr) => attr.value_len(),
            ConfigNlas::QThresh(_) => size_of::<u32>(),
            ConfigNlas::Flags(ref attr) => attr.value_len(),
            ConfigNlas::Other(ref attr) => attr.value_len(),
        }
    }

    fn kind(&self) -> u16 {
        match self {
            ConfigNlas::Cmd(ref attr) => attr.kind(),
            ConfigNlas::Mode(ref attr) => attr.kind(),
            ConfigNlas::NlBufSiz(_) => NFULA_CFG_NLBUFSIZ,
            ConfigNlas::Timeout(ref attr) => attr.kind(),
            ConfigNlas::QThresh(_) => NFULA_CFG_QTHRESH,
            ConfigNlas::Flags(ref attr) => attr.kind(),
            ConfigNlas::Other(ref attr) => attr.kind(),
        }
    }

    fn emit_value(&self, buffer: &mut [u8]) {
        match self {
            ConfigNlas::Cmd(ref attr) => attr.emit_value(buffer),
            ConfigNlas::Mode(ref attr) => attr.emit_value(buffer),
            ConfigNlas::NlBufSiz(buf_siz) => BigEndian::write_u32(buffer, *buf_siz),
            ConfigNlas::Timeout(ref attr) => attr.emit_value(buffer),
            ConfigNlas::QThresh(q_thresh) => BigEndian::write_u32(buffer, *q_thresh),
            ConfigNlas::Flags(ref attr) => attr.emit_value(buffer),
            ConfigNlas::Other(ref attr) => attr.emit_value(buffer),
        }
    }
}
